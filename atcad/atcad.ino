...
#define button2 P4_0
#define button3 P3_7
#define lock_button P4_3
//vibration pin Configuration
#define vibraton_pin P3_5
...
char pnrdata[10][4][20]; //storage for pnr numbers history
int current_pnr_no=0;
char ttedata[10][20]; //storage for pnr numbers history
...
int real_hrs;
//
unsigned long reminder_time;//store alarm sec
unsigned long reminder_sec; //
unsigned long reminder_mint;
unsigned long reminder_hrs;
boolean reminder_status=false;
...
void automatic_bacllight(void){
  ...
  if(backlight_status==true){
    ...
    lcd_backlight_value=lcd_backlight_value/7;
    lcd_backlight_value=255-lcd_backlight_value;
    if(lcd_backlight_value<0) lcd_backlight_value=0;
    if(backlight_value<lcd_backlight_value) backlight_value=backlight_value+5;
    if(backlight_value>lcd_backlight_value) backlight_value=backlight_value-5;
    analogWrite(P1_4,backlight_value);
  }
...
void eva_post(String query_url, String query_statement,String parameter,String request_category,String Content_Length )
{
  t=0;
  Serial1.flush();
  lcd.clear();
...
void print_menu(char *line1,char *line2,char *line3){
  lcd.clear();
  lcd.setCursor(0,0);
  lcd.print(line1);
  lcd.setCursor(0,1);
...
void settings(void){
  ...
  char setting_menu[][20]={
    "1-BACKLIGHT","2-AUTOMATIC LOCK","3-VIBRATION","4-TIME","5-MCU DETAILS","6-NO OPTION"};
setting_menu:
  Serial1.flush();
  if(i<0) i=5;
  if(i<4){
...
automatic_bacllight_setting: 
  ...
  case 'C':
    ...
    case true:
      print_menu("AUTOMATIC BACKLIGHT:","OFF","");
      backlight_status=false;
      break;

    case false:
      print_menu("AUTOMATIC BACKLIGHT:","ON","");
...
void time_settings(void){
  ...
  char time_settings_menu[][20]={
    "1-Set Current Time","2-Set Reminder","3-Cancel Reminder","4-Request For Time"                              };
time_settings:
  Serial1.flush();
  if(i<2){
    w=i;
  ...
  case 'C':
    ...
    case 3:
      lcd.clear(); 
      if(reminder_status==true)  lcd.print("Reminder :Disable");
      if(reminder_status==false){ 
        lcd.print("REMINDER :No"); 
        reminder_status=false;
      }
      delay(1000);
      break;
...
void set_reminder(void){
  ...
  lcd.noBlink();
  time=millis();
  reminder_time=reminder_sec+reminder_mint*60+reminder_hrs*3600;
  reminder_time=reminder_time+time/1000;
  reminder_status=true;
  delay(500);
  return;
}

void mc_details(void){  
  if(automatic_lock_status==true) get_lock_time();
  i=0;
function1:  
  print_menu("1-CPU Uptime","2-CPU Temperature","3-NO OPTION");//verver
  lcd.setCursor(0,3); 
  lcd.write(2); 
  lcd.write(2);  
  lcd.write(2); 
...
void display_uptime(){
  ...
  while(digitalRead(button1)==HIGH){
    ...
    if(reminder_status==true){
      ...
      lcd.print(reminder_time/3600);
      lcd.print(":");
      lcd.print((reminder_time/60)-60*(reminder_time/3600));
    }
    else
      lcd.print("REMINDER:NO");
...
void internal_temp(void){
  ...
  lcd.write(2);
  if(automatic_lock_status==true) get_lock_time();
  print_menu("Internal Temp","Cel:","Cel:");
...
void function3(void){
  ...
  i=0;
function3:  
  print_menu("1-MAKE CONNECTION","2-DEVICE VERIFICATION","3-TURN OFF DEVICE");//verver
  lcd.setCursor(0,3); 
  lcd.write(2); 
  lcd.write(2);  
  lcd.write(2); 
  ...
  case 'C':
    ...
    case 1:
      ...
      if(data[0][6]=='F'){
      lcd.clear();
      lcd.print("Please Try Another Card");
      delay(2000);
      get_tte_number();
      goto top;
      }
...
main_menu:
  ...
  case 'C':
    ...
    case 3:
      device_verification(posturl,"DEVICE VERIFICATION:","evaluate_request","81");
      post_result();
      break;

    case 4:
      eva_post(posturl,"STATION No.","station_code=","evaluate","88");
      post_result();
      break;
