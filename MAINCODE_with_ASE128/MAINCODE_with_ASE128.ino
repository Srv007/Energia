...
int device_lock(void){
  ...
  String user_enter_password=String(4);
  String code=String(12);
  lcd.clear();
  lcd.print("        LOCK");
  delay(500);
  ...
  case '1':
    ...
    for  (i=0;i<4;i++){
      a = keylock();
      lcd.print(a);
      user_enter_password.setCharAt(i,a) ;	
    }
    lcd.print(user_enter_password);
    delay(10000);
    lcd.noBlink();
    lcd.setCursor(0, 1);
    digitalWrite(vibraton_pin,LOW);
...
void function1(void){  
  ...
  i=0;
function1:  
  print_menu("1-CPU Uptime","2-CPU Temperature","3-pta nhi","");//verver
  lcd.setCursor(0,3); 
  lcd.write(2); 
  lcd.write(2);  
  lcd.write(2); 
